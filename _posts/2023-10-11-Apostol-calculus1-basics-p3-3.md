...
如果我们用$<$ 代替 (I.17) 中的不等号$\le$ ，就得到小数展开式的一个稍微不同的定义。虽然得到的整数$a_0, a_1, a_2,\cdots$ 可能与$\le$的定义不同，但形如(I.18) 的所有近似值的集合$S$ 的最小上界仍然是$x$.  例如，如果对$x=\frac{1}{8}$ 应用第二个定义，我们选定$a_0=0,a_1=1,a_2=2,a_3=4$, 对所有$n\ge 4$ $a_n=9$.  这就得到了$\frac{1}{8}$ 的无尽小数展开式
